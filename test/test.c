...
static void test_seed_from_string() {
  ...
  uint8_t ref_secret[] = {0x8d, 0x96, 0x9e, 0xef, 0x6e, 0xca, 0xd3, 0xc2,
                          0x9a, 0x3a, 0x62, 0x92, 0x80, 0xe6, 0x86, 0xcf};
  size_t strings_len = 12;
  char *ref_strings[] = {"mirror",  "reject",  "rookie",  "talk",
                         "pudding", "throw",   "happy",   "era",
                         "myth",    "already", "payment", "owner"};
  uint16_t words[] = {1132, 1447, 1502, 1772, 1385, 1802,
...
static bool _test_mnemonics_from_secret(const char *secret_hex,
                                        const char *expected_mnemonics) {
  uint8_t *secret_data;
  size_t secret_len = hex_to_data(secret_hex, &secret_data);
  size_t max_mnemonics_len = 300;
  char mnemonics[max_mnemonics_len];
  size_t mnemonics_len = bip39_mnemonics_from_secret(
      secret_data, secret_len, mnemonics, max_mnemonics_len);
  bool result;
...
static void test_mnemonics_from_secret() {
  assert(_test_mnemonics_from_secret(
      "baadf00dbaadf00d", "rival hurdle address inspire tenant alone"));
...
static bool _test_secret_from_mnemonics(const char *mnemonics,
                                        const char *expected_secret_hex) {
  uint8_t *expected_secret;
  size_t expected_secret_len =
      hex_to_data(expected_secret_hex, &expected_secret);

  size_t max_secret_len = 32;
  uint8_t secret[max_secret_len];
  size_t secret_len =
      bip39_secret_from_mnemonics(mnemonics, secret, max_secret_len);

  bool result = equal_uint8_buffers(secret, secret_len, expected_secret,
                                    expected_secret_len);
...
static void test_secret_from_mnemonics() {
  assert(_test_secret_from_mnemonics(
      "rival hurdle address inspire tenant alone", "baadf00dbaadf00d"));
  assert(_test_secret_from_mnemonics("rival hurdle address inspire tenant "
                                     "almost turkey safe asset step lab boy",
                                     "baadf00dbaadf00dbaadf00dbaadf00d"));
  assert(_test_secret_from_mnemonics("legal winner thank year wave sausage "
                                     "worth useful legal winner thank yellow",
