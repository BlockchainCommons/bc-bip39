...
size_t hex_to_data(const char *hex, uint8_t **out) {
  ...
  *out = malloc(len);
  for (size_t i = 0; i < len; i++) {
    char b1;
    char b2;
    if (!hex_digit_to_bin(hex[i * 2], &b1) ||
        !hex_digit_to_bin(hex[i * 2 + 1], &b2)) {
      return 0;
    }
    (*out)[i] = (b1 << 4) | b2;
  }
...
bool equal_strings(const char *a, const char *b) { return strcmp(a, b) == 0; }
...
bool equal_uint8_buffers(const uint8_t *buf1, size_t len1, const uint8_t *buf2,
                         size_t len2) {
  if (len1 != len2) {
    return false;
  }
...
bool equal_uint16_buffers(const uint16_t *buf1, size_t len1,
                          const uint16_t *buf2, size_t len2) {
  if (len1 != len2) {
    return false;
  }
